function params = Dfind_crossings(params, direction,ectopic_status)

%D based on find_crossings.m but altered as follows:
%D for FTOC use either the major, minor or mean collicular
%D position for the ectopic projections
%D values of ectopic_status are 'MAJOR', 'MINOR';
%D  Using any other string gives mean position
	...
	params.FTOC.mean_projection(ect,:) = projected_points(ect,:);
	projected_points(ect,:)
	if ectopic_status == 'MAJOR'
	   projected_points(ect,:) =  params.FTOC.major_projection(ect,:);
	end
	if ectopic_status == 'MINOR'
	   projected_points(ect,:) =  params.FTOC.minor_projection(ect,:);
	end
	projected_points(ect,:)
