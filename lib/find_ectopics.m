function params = find_ectopics(params,direction)
...
%    adjust because of how axes are drawn
     ...
     ect_angles;
    params.FTOC.stats.mean_ectopic_angles=mean(ect_angles);
    params.FTOC.stats.std_ectopic_angles=std(ect_angles,1);
    ...
    params.stats.FTOC.num_ectopics = length(ect)
    ect_dists = diag(compute_dist(major_projection(ect,:),minor_projection(ect,:)'));
    params.stats.FTOC.ect_dist_mean = mean(ect_dists);
    params.stats.FTOC.ect_dist_std = std(ect_dists,1);
%D   added by David
    params.stats.FTOC.ectopics=ect;
...
if strcmp(direction,'CTOF')
    ...
    major_projection = params.CTOF.field_points;
    minor_projection = zeros(numpoints,2);


    for point = 1:numpoints
        centre = coll_points(point,:);
...
%        (ii)  distance between means  > summed standard deviations 
    ...
    [major_projection minor_projection, optimal_position] = find_best_position(coll_points,major_projection, minor_projection, takeout );
...
%    adjust because of how axes are drawn
     ...
     ect_angles;
    params.stats.CTOF.mean_ectopic_angles=mean(ect_angles);
    params.stats.CTOF.std_ectopic_angles=std(ect_angles,1);

    params.CTOF.major_projection = major_projection;
    params.CTOF.minor_projection = minor_projection;
    params.CTOF.optimal_position = optimal_position;

    ect = find(minor_projection(:,1));
    params.stats.CTOF.num_ectopics = length(ect);
    ect_dists = diag(compute_dist(major_projection(ect,:),minor_projection(ect,:)'));
    params.stats.CTOF.ect_dist_mean = mean(ect_dists);
    params.stats.CTOF.ect_dist_std = std(ect_dists,1);
%D   added by David
    params.stats.CTOF.ectopics=ect;
end
